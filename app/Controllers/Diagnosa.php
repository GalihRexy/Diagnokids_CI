...
class Diagnosa extends BaseController
{
	...
	public function index()
	{
		if(session()->has('email')){
			...
			$data['name'] = session('name');
 			...
 			// var_dump(count($data['anak']) != null);die();
	...
	{
		session()->set('id_anak', $id);
		$keluhanModel = new KeluhanModel();
		$data['keluhan'] = $keluhanModel->findAll();
	...
	public function diagnosa_gejala()
	{
		...
		if (isset($keluhan)) {
			for ($i = 0; $i < count($keluhan); $i++) {
				if ($keluhan[$i] == 'K1') {
					session()->setFlashdata('K1', 'K1');
				} else if ($keluhan[$i] == 'K2') {
					session()->setFlashdata('K2', 'K2');
				} else if ($keluhan[$i] == 'K3') {
					session()->setFlashdata('K3', 'K3');
				}
			}	
		}
	...
	public function diagnosa_proses()
	{
		...
		// Definikan variable
		$diagnosaModel = new DiagnosaModel();
		$dDetail = new DiagnosaDetailModel();
		$email = session('email');
		$id_anak = session('id_anak');
		...
		return redirect()->to(base_url('data-diagnosa'));
	}
