...
<body>
	  ...
	  <div class="container">
		  ...
		  <div class="collapse navbar-collapse" id="navbarSupportedContent">
		    <ul class="navbar-nav ml-auto align-self-center">
		      <li class="nav-item">
		        <a class="nav-link mt-1" href="<?= base_url('home'); ?>">Home</a>
		      </li>
		      <li class="nav-item">
		        <a class="nav-link mt-1" href="<?= base_url('profile') ?>">User Profile</a>
		      </li>
		      <li class="nav-item">
		        <a class="nav-link mt-1 " href="<?= base_url('diagnosa') ?>">Diagnosa</a>
		      </li>
		      <li class="nav-item">
		        <a class="nav-link mt-1 active" href="<?= base_url('data-diagnosa') ?>">Data Diagnosa</a>
		      </li>
		      <button class="btn-nav-secondary mr-3 ml-5 align-self-center"><a href="<?= base_url('auth/logout'); ?>" style="text-decoration: none; color: #232D42;">Sign Out</a></button>
	...
	<section id="diagnosaDetail" class="container mt-5">
		...
		<p class="mt-n3"><b>Nama:</b> <?= $diagnosaModel->getNama($diagnosa['id_anak']); ?></p>
		<div class="mt-5">
			<p>Berdasarkan diagnosa yang telah dilakukan anak anda memiliki gejala berupa:</p>
			<ul>
				<?php foreach($detail as $row) { ?>
				<li><?= $diagnosaDetailModel->getGejala($row['kode_gejala']); ?></li>
				<?php } ?>
			</ul>
		</div>
		<div>
			...
			<ul>
				<?php foreach($detail as $row) { ?>
				<li><?= $diagnosaDetailModel->getPenyakit($row['kode_gejala']); ?></li>
				<?php } ?>
			</ul>
		</div>
	</section>
